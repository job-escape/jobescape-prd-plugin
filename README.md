...
4. The final PRD is delivered as **two files**: `prd.yaml` — a structured, machine-first document (typed requirement rows with screen/component/state/trigger/outcome/design-node fields, a flow map of forward and return edges, F-step groupings, per-screen design status, a reuse-of-existing-systems section, and open items cross-referenced to the rows they block; stable R/NG/S/Q/F IDs throughout) — plus `prd.md`, a human-readable render generated from it (the YAML is canonical). During the interview you review normal tables and lists; the YAML compile happens at the end. Drafts and the final YAML are checked by the audits plus a mechanical linter (dangling TBD references, broken ID sequences, missing owners, unreturned flow edges).
